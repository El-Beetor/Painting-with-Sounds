...
public class pxStrax : MonoBehaviour {
    ...
    public float Run()
    {
        lfo.frequency = lfoRate;
        lfo.amp = lfoAmp;
        level = lope.Run() * FilterRun(StraxRun()); //sends our Square and triangle wave through a filter and multiplies that with our LFO
        return level;
    }
    ...
    public float StraxRun()
    {
        ...
        return 1.0f - ((Mathf.Floor(px1+0.5f)*2f-1f) * (1f - osc2Mix) + (Mathf.Abs(1f-(px2*2f))*2f-1f) * osc2Mix); //SquareWave and Triangle Wave
    }
    ...
    {
        float x = waveIn;
        float distx = Mathf.Clamp(distortion, 0.000f, 1.0f); // sets our distortion in a range
    ...
    public void KeyOn(float midinote)// Gets MidiNote from Mouse Strax and starts attack
    {
        SetNote(midinote);
        lope.SetParams(attack, release, sustain);
        lope.KeyOn();
    ...
    public void KeyOff() //Starts release
    {
        lope.KeyOff();
    }
